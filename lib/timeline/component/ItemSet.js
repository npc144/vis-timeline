 ...
 */
  ...
  _onGroupClick(event) {
    const group = this.groupFromTarget(event);
    group.preventToggleGroupShowNested = false;

    this.body.emitter.emit('groupClick', { group });

    if (!group.preventToggleGroupShowNested) {
      setTimeout(() => {
        this.toggleGroupShowNested(group);
        this.body.emitter.emit('groupClicked', { group, defaultEvent: event });
      }, 1)
    }
  }
  ...
  _onSelectItem(event) {
    ...
    const selection = item != null ? [item.id] : [];
  ...
  _onMultiSelectItem(event) {
    ...
    let item = this.itemFromTarget(event);
    ...
    if (item) {
      ...
      if ((shiftKey || this.options.sequentialSelection) && this.options.multiselect) {
        ...
        // Select last element of the cluster if we clicked one
        if (item.isCluster && item.data.items.length > 0)
            item = item.data.items[item.data.items.length - 1];
        ...
        if (index == -1) {
          ...
          if (item.isCluster) {
            let i = item.data.items.length - 1;

            while (i >= 0) {
              index = selection.indexOf(item.data.items[i].id);
              if (index !== -1) {
                selection.splice(index, 1);
              }
              --i;
      ...
      const filteredSelection = selection.filter(item => {
        const currentItem = this.getItemById(item);
        if (currentItem)
  ...
  _updateClusters(clusters) {
    if (this.clusters && this.clusters.length) {
      ...
      const clustersToSelect = clusters.filter(cluster => cluster.data.uiItems.some(x => x.selected));
      ...
      for (let cluster of clustersToSelect) {
        cluster.select();
        this.selection.push(cluster.id);
      }
    }
