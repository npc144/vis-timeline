 ...
 */
  ...
  _calculateHeight(margin) {
    ...
    if (items.length > 0) {
      let min = items[0].top || 0;
      let max = (items[0].dom != null && items[0].dom.box ? items[0].dom.box.offsetHeight : 0) || items[0].top + items[0].height || 0;
      util.forEach(items, item => {
        if (item.displayed && !item.isCluster) {
          min = Math.min(min, item.top);
          max = Math.max(max, item.top + item.height);

          if (item.dom && item.dom.box)
            max = Math.max(max, item.dom.box.offsetTop + item.dom.box.offsetHeight);
        }
      });
      if (min > margin.axis) {
